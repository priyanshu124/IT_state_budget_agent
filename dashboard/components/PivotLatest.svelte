<script>
    ...
    const conditionalStyle = (v) => {
        if (v == null) return 'display:inline-block;padding:4px 8px;border-radius:4px;min-width:54px;text-align:right;color:#6B7280;';
        const n = Number(v);
...
<div style="margin-bottom:12px;">
    <input
        ...
        style="border:1px solid #D9DDE3; border-radius:8px; padding:8px 12px; font-size:0.9rem; width:340px;"
    />
</div>
        ...
        <tbody>
            {#each filteredUnits as unit, i}
                <tr
                    ...
                    style={'border-bottom:1px solid #E5E7EB; cursor:pointer; background:' + (i % 2 === 0 ? 'white' : '#FAFAFA') + ';'}
                        ...
                        {#if expandedPrograms[unit.unit_name + '||' + prog.program_name]}
                            {#each getFilteredSubprograms(unit.unit_name, prog.program_name) as sub}
                                <tr style="border-bottom:1px solid #F3F4F6; background:#FAFAFA;">
                                    <td style="padding:7px 14px 7px 60px; color:#6B7280; font-style:italic;">{sub.subprogram_name}</td>
