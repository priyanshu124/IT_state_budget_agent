<script>
    ...
    const setSort = (col) => {
        if (sortCol === col) {
            ...
            // Numeric columns (money, percent, conditional) default to descending
            // String columns default to ascending
            sortDir = (colDef?.fmt || colDef?.conditional) ? -1 : 1;
        }
    };
    ...
    $: sorted = sortCol
        ? filtered.slice().sort((a,b) => {
            ...
            return sortDir * (Number(aVal) - Number(bVal));
        })
        : filtered;
                    ...
                    {#each columns as col}
                        <td style={'padding:9px 14px; text-align:' + (col.align ?? 'right') + '; ' + (col.bold ? 'font-weight:600;' : '') + ' color:#231F20'}>
                            ...
                            {:else}
                                ...
                                {:else}
                                    {col.fmt === 'money' ? fmtMoney(row[col.id]) : col.fmt === 'pct' ? fmtPct(row[col.id], false) : (row[col.id] == null ? '-' : row[col.id])}
                                {/if}
                            {/if}
                        </td>
                    {/each}
